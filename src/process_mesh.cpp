...
{
    std::map<Vertex_t, unsigned, compareVertices> vertexInfo;
    int numVertices =  in_vertices.size()/vertex_size;
    int numTriangles = numVertices/3;

    int in_vertexID = 0;
    int out_vertexID = 0;

    std::cout << "numTriangles = " << numTriangles << ", numVertices = " << numVertices << std::endl;

    for (int t = 0; t < numTriangles; ++t) {

        int va_idx = t*3*vertex_size;
        int vb_idx = va_idx + vertex_size;
        int vc_idx = vb_idx + vertex_size;
        ...
        for (int v = 0; v < 3; v++) {
            ...
            std::cout << "vertex " << t*3 + v << ": " << key.x << " " << key.y << " " << key.z << std::endl;
            ...
            if (vertexInfo.find(key) == vertexInfo.end()) {
                ...
                std::cout << "found " << vertexInfo[key] << std::endl;
                    ...
                    std::vector<float> &normals)
{
    ...
    for (int i = 0; i < numTriangles; ++i) {
        ...
        Vertex_t v0 = {vertices[vidx0*4+0], vertices[vidx0*4+1], vertices[vidx0*4+2]};
        Vertex_t v1 = {vertices[vidx1*4+0], vertices[vidx1*4+1], vertices[vidx1*4+2]};
        Vertex_t v2 = {vertices[vidx2*4+0], vertices[vidx2*4+1], vertices[vidx2*4+2]};

        Vertex_t e1 = v1 - v0;
        Vertex_t e2 = v2 - v0;
        Vertex_t cr = cross(e1,e2);

        normals[vidx0*4+0] += cr.x;
        normals[vidx0*4+1] += cr.y;
        normals[vidx0*4+2] += cr.z;

        normals[vidx1*4+0] += cr.x;
        normals[vidx1*4+1] += cr.y;
        normals[vidx1*4+2] += cr.z;

        normals[vidx2*4+0] += cr.x;
        normals[vidx2*4+1] += cr.y;
        normals[vidx2*4+2] += cr.z;
    }
    for (int i = 0; i < normals.size()/4; ++i) {
        Vertex_t n = {normals[i*4], normals[i*4+1], normals[i*4+2]};
        n = normalize(n);
    }
}
